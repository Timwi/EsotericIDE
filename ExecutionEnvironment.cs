...
namespace EsotericIDE
{
    ...
    abstract class ExecutionEnvironment : IDisposable
    {
        ...
        protected void fireExecutionFinished(bool canceled) { if (ExecutionFinished != null) ExecutionFinished(canceled); }
        ...
        public void AddBreakpoint(int index) { lock (_locker) { _breakpoints.Add(index); } if (BreakpointsChanged != null) BreakpointsChanged(); }
        public bool RemoveBreakpoint(int index) { bool ret; lock (_locker) { ret = _breakpoints.Remove(index); } if (BreakpointsChanged != null && ret) BreakpointsChanged(); return ret; }
        public void ClearBreakpoints() { lock (_locker) { _breakpoints.Clear(); } if (BreakpointsChanged != null) BreakpointsChanged(); }
